 ...
 *  but they become directly opposite after the reduction of "EAST" and "WEST" so the whole path is 
...
public class DirReduction {
    ...
    public static String[] listToArr(List<String> arr) {
    ...
    public static String[] dirReduc(String[] arr) {
        ...
        List<String> arrList = new ArrayList<>(directions);

        for (int i = 0; i < arr.length - 1; i++) {
            if ((arrList.get(i) == "NORTH" && arrList.get(i + 1) == "SOUTH") ||
                (arrList.get(i) == "SOUTH" && arrList.get(i + 1) == "NORTH")) {
                        arrList.remove(i);
                        arrList.remove(i);

                        return dirReduc(listToArr(arrList));
                }

            else if ((arrList.get(i) == "WEST" && arrList.get(i + 1) == "EAST") ||
                    (arrList.get(i) == "EAST" && arrList.get(i + 1) == "WEST")) {
                        arrList.remove(i);
                        arrList.remove(i);
